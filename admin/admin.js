...
async function loadAdminPolls() {
  const container = document.getElementById('polls-admin-container');
  container.innerHTML = '<p style="color:var(--muted);font-size:0.8rem">Loading...</p>';

  const { data: polls } = await db.from('polls').select('*').order('created_at', { ascending: false });

  if (!polls || polls.length === 0) {
    container.innerHTML = '<p style="color:var(--muted);font-size:0.8rem">No polls yet.</p>';
    return;
  }
  ...
  const { data: allResponses } = await db
    .from('poll_responses')
    .select('poll_id, response, members(first_name, surname, member_code)')
    .in('poll_id', polls.map(p => p.id));

  const byPoll = {};
  (allResponses || []).forEach(r => {
    if (!byPoll[r.poll_id]) byPoll[r.poll_id] = { yes: [], maybe: [], no: [] };
    const name = r.members ? `${r.members.first_name} ${r.members.surname || ''}`.trim() : '?';
    byPoll[r.poll_id][r.response]?.push(name);
  });

  container.innerHTML = polls.map(p => {
    const s = byPoll[p.id] || { yes: [], maybe: [], no: [] };
    const total = s.yes.length + s.maybe.length + s.no.length;
    return `
      <div style="background:var(--surface);border:1px solid ${p.active ? 'var(--gold)' : 'var(--border)'};padding:1.25rem;margin-bottom:0.75rem">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;flex-wrap:wrap">
          <div style="flex:1">
            <div style="font-family:var(--font-serif);font-size:1rem;margin-bottom:0.25rem">${p.title}</div>
            ${p.title_afr ? `<div style="font-size:0.7rem;color:var(--muted);margin-bottom:0.5rem">AFR: ${p.title_afr}</div>` : ''}
            <div style="display:flex;gap:1.5rem;margin:0.75rem 0;flex-wrap:wrap">
              <div style="font-size:0.8rem"><span style="color:#6bbf80">✓ Ja</span> · ${s.yes.length}</div>
              <div style="font-size:0.8rem"><span style="color:var(--gold)">~ Dalk</span> · ${s.maybe.length}</div>
              <div style="font-size:0.8rem"><span style="color:#c0605a">✗ Nee</span> · ${s.no.length}</div>
              <div style="font-size:0.7rem;color:var(--muted)">${total} response${total !== 1 ? 's' : ''}</div>
            </div>
            ${s.yes.length > 0 ? `<div style="font-size:0.7rem;color:var(--muted);margin-top:0.25rem"><strong style="color:#6bbf80">Ja:</strong> ${s.yes.join(', ')}</div>` : ''}
            ${s.maybe.length > 0 ? `<div style="font-size:0.7rem;color:var(--muted);margin-top:0.25rem"><strong style="color:var(--gold)">Dalk:</strong> ${s.maybe.join(', ')}</div>` : ''}
            ${s.no.length > 0 ? `<div style="font-size:0.7rem;color:var(--muted);margin-top:0.25rem"><strong style="color:#c0605a">Nee:</strong> ${s.no.join(', ')}</div>` : ''}
          </div>
          <div style="display:flex;gap:0.5rem;flex-direction:column;align-items:flex-end">
            <span class="badge-small ${p.active ? 'green' : ''}">${p.active ? 'Active' : 'Closed'}</span>
            <button class="btn-admin" onclick="togglePollActive('${p.id}', ${p.active})">${p.active ? 'Close' : 'Reopen'}</button>
            <button class="btn-admin danger" onclick="deletePoll('${p.id}')">Delete</button>
          </div>
        </div>
      </div>`;
  }).join('');
...
function openAddPoll() {
  document.getElementById('modal-content').innerHTML = `
    ...
    <div class="form-group"><label>Question (English)</label><input id="poll-title-en" placeholder="Are you in town this weekend?" /></div>
    <div class="form-group"><label>Question (Afrikaans)</label><input id="poll-title-afr" placeholder="Is jy hierdie naweek in die dorp?" /></div>
    <div class="form-group"><label>Description (optional, English)</label><textarea id="poll-body-en" placeholder="We're thinking of organising something informal..."></textarea></div>
    <div class="form-group"><label>Description (optional, Afrikaans)</label><textarea id="poll-body-afr"></textarea></div>
    <div class="form-actions">
      <button class="btn-admin" onclick="closeModal()">Cancel</button>
      <button class="btn-admin primary" onclick="savePoll()">Post Poll</button>
    </div>
  `;
  document.getElementById('modal-overlay').classList.remove('hidden');
}
...
async function savePoll() {
  const titleEn = document.getElementById('poll-title-en').value.trim();
  const titleAfr = document.getElementById('poll-title-afr').value.trim();
  if (!titleEn && !titleAfr) { showToast('Please enter a question'); return; }

  await db.from('polls').insert({
    title:     titleEn  || titleAfr,
    title_afr: titleAfr || titleEn,
    body:      document.getElementById('poll-body-en').value.trim()  || null,
    body_afr:  document.getElementById('poll-body-afr').value.trim() || null,
    active: true,
  });
...
async function togglePollActive(id, current) {
  await db.from('polls').update({ active: !current }).eq('id', id);
  loadAdminPolls();
}

async function deletePoll(id) {
  if (!confirm('Delete this poll and all responses?')) return;
  await db.from('poll_responses').delete().eq('poll_id', id);
